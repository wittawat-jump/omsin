...
function initIerecord() {
  var doStatusChanged = function () {
    if (this.value == 'TRANSFER') {
      ...
      $G('write_category').setValue('').reset().focus();
      $E('write_wallet_name').parentNode.parentNode.style.display = 'none';
...
function findCategory(name, owner_id, typ, count) {
  ...
  function SearchPopulate() {
    ...
    return '<p><span>' + this.name.unentityify().replace(patt, '<em>$1</em>') + '</span></p>';
  }
...
var doDatabaseReset = function () {
  if (confirm(CONFIRM_RESET_DATABASE)) {
    if (confirm(CONFIRM_RESET_DATABASE_2)) {
      send(WEB_URL + 'xhr.php/index/model/database/action', 'action=reset', doFormSubmit);
    }
  }
};